...
function SeedPage() {
  ...
  async function handleSeed() {
    ...
    try {
      ...
      setResult({ ok: false, created: [], skipped: [], failed: [], error: e?.message });
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          ...
          progress + submissions. Safe to run more than once — it skips anything that already exists.
        </p>
      </header>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Creates accounts (password <code className="rounded bg-muted px-1.5 py-0.5">Academy@123</code>),
            franchises, courses, lessons, assignments and a few graded practical submissions so the Incharge
            queue and Member dashboard are populated immediately.
          </CardDescription>
        </CardHeader>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <div>
                ...
                <CardDescription>Account creation results.</CardDescription>
              </div>
            </CardHeader>
            <CardContent className="grid grid-cols-3 gap-3 text-sm">
              <Stat label="Created" value={result.created.length} tone="success" />
              <Stat label="Already existed" value={result.skipped.length} tone="muted" />
              <Stat label="Failed" value={result.failed.length} tone={result.failed.length ? "danger" : "muted"} />
            </CardContent>
          </Card>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0">
                <div>
                  ...
                  <CardDescription>
                    All accounts share the same password. Copy and try them out — sign out and log in as
                    Incharge or as a Member to see different views.
                  </CardDescription>
                </div>
