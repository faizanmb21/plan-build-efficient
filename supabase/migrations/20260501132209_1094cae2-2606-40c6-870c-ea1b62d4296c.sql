...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      bucket := (i * 17 + abs(hashtext(member_rec.user_id::text)) + abs(hashtext(lesson_rec.lesson_id::text))) % 100;

      IF bucket < 20 THEN
        -- Pending (awaiting QA/Incharge review)
  ...
  -- Ensure each member is assigned to every published course (so the matrix has
  -- something to render even where seed_demo_content didn't loop).
